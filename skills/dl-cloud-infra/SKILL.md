...
When running inside the devline pipeline via the devops agent, this skill guides infrastructure work packages. The devops agent follows TDD (via the dl-tdd-workflow skill) for infra changes too — write validation scripts or smoke tests before making changes. For example: write a test that checks a Docker image builds and starts correctly before writing the Dockerfile.
...
Use the context7 MCP server for current cloud SDK and service documentation.
