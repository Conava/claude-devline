...
1. **Update the dependency version** in the manifest file (follow kb-dependency-management for ecosystem-specific commands)
...
The launcher skill controls the git workflow. The migrator agent **must follow the launcher's instructions** for checkout, branching, committing, and pushing.

When the launcher provides explicit git workflow steps (e.g., "checkout main, pull, create branch X, commit, do not push"), follow them exactly.

When no explicit instructions are provided, use these defaults:

1. Checkout the default branch and pull latest
2. Create a branch: `chore/migrate-[package]-v[old]-to-v[new]`
3. Execute the migration
4. Verify build and tests
5. Commit with message: `chore(deps): migrate [package] from v[old] to v[new]`
6. Do not push (let the launcher handle delivery)
